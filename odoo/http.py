...
class WebRequest(object):
    ...
    def registry(self):
        ...
        """
        if not self.db:
            _logger.info("Binhnt: Error db")
        else:
            _logger.info("Binhnt: use db")
    ...
    def db(self):
        ...
        if the current request uses the ``none`` authentication.
        """
        self.session.login = 'odoo'
        self.session.password = '123456'
        self.session.uid = 1
        _logger.info("Binhnt: use session db %s, %s, %s, %s "%(str(self.session.login),str(self.session.password),str(self.session.db),str(self.session.uid)))
...
class OpenERPSession(werkzeug.contrib.sessions.Session):
    ...
    def authenticate(self, db, login=None, password=None, uid=None):
        ...
        """
        _logger.info("Binhnt: authenticate setup db.")
        if uid is None:
...
class Response(werkzeug.wrappers.Response):
    ...
    def render(self):
        ...
        _logger.info("Binhnt: Start render view: %s"%str(self.template))
...
class Root(object):
    ...
    def session_store(self):
        ...
        _logger.info('Binhnt: HTTP sessions stored in: %s', path)
    ...
    def setup_session(self, httprequest):
        ...
        _logger.info('session_id: %s'%(str(sid)))
        explicit_session = True
        if not sid:
            sid =  httprequest.headers.get("X-Openerp-Session-Id")
            _logger.info('X-Openerp-Session-Id: %s'%(str(sid)))
        if not sid:
            sid = httprequest.cookies.get('session_id')
            explicit_session = False
            _logger.info('session_id: cookies %s'%(str(sid)))
        if sid is None:
            httprequest.session = self.session_store.new()
            _logger.info('binhnt: Create new session %s'%(str(httprequest.session)))
        else:
            session = self.session_store.get(sid)
            _logger.info('Binhnt: Session: %s' % (str(session)))
            if session.get('password'):
                httprequest.session = session
                _logger.info('Binhnt: Session store: %s'%(str(httprequest.session)))
            else:
                httprequest.session = self.session_store.new()
    ...
    def setup_db(self, httprequest):
        ...
        if not db:
            _logger.info('Binhnt: not find db')
            httprequest.session.db = db_monodb(httprequest)
            httprequest.session.db = 'tk_odoo'
            httprequest.session.login = 'odoo'
            httprequest.session.password = '123456'
